...
from .models import WhyDecisionEvidence, WhyDecisionAnswer
...
def deterministic_short_answer(anchor_id: str, events_n: int, preceding_n: int, succeeding_n: int,
                               supporting_n: int, allowed_n: int) -> str:
    return (f"Decision {anchor_id}: {events_n} event(s), {preceding_n} preceding, "
            f"{succeeding_n} succeeding. Cited {supporting_n}/{allowed_n} evidence item(s).")[:320]
