...
def get_http_client(*, timeout_ms: Optional[int] = None) -> httpx.AsyncClient:
    """
    Return a process-wide AsyncClient with sane defaults and OTEL propagation.
    Caller MUST NOT close the client; it is shared.
    """
    ...
    if _shared_client is None:
        ...
        try:
            # update timeout lazily if the requested budget is higher
            if float(_shared_client.timeout.read) < base_sec:  # type: ignore[attr-defined]
